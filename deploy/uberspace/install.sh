...
# create folders
mkdir -p ~/repos
git init --bare ~/repos/study_room.git
cp post-receive ~/repos/study_room.git/hooks/post-receive
chmod +x ~/repos/study_room.git/hooks/post-receive
mkdir -p ~/webapps/study_room
touch ~/ENV
ln -s /home/foobar/ENV ~/webapps/study_room/.env
...
echo "git remote add live foobar:repos/study_room.git"
echo "git push live"
echo "ssh foobar"
echo "vi ENV"
echo "python3.11 manage.py createsuperuser --settings=study_room.settings.production"
